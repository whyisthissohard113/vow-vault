 ...
 * `{organizationId}/{weddingId}/{mediaId}.{ext}` — never derived from user
 * input. Variants append the variant type so all renditions stay in the same
 * tenant namespace (defense-in-depth for signed-URL scoping).
 */
...
export function buildObjectKey(input: ObjectKeyInput): string {
  ...
  return `${input.organizationId}/${input.weddingId}/${input.mediaId}${ext}`;
}
...
export function buildVariantObjectKey(input: VariantObjectKeyInput): string {
  const ext = normalizeExt(input.ext);
  return `${input.organizationId}/${input.weddingId}/${input.mediaId}.${input.variantType}${ext}`;
}
